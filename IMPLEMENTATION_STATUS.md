...
## UI גלוי תחת הדומיין (חדש — עמודים אמיתיים)

| עמוד | URL | מה מציג | סטטוס |
|---|---|---|---|
| התוכנית שלי | /program | 20 יחידות תוכן + פתיחת שיעור מלא (מטרה/תרגיל/תמלול/רפלקציה/safety) | COMPLETE — חי, מחובר ל-API + fallback סטטי |
| קטי הדיגיטלית | /talk | צ׳אט חי (/api/checkin), ציטוט מקור, "לא מתאים לי"/"אני צריכה אדם", בטיחות | COMPLETE — חי, נבדק E2E מול הבקאנד |
| היום שלי | /today | הצעד הבא + שיחה/תוכנית + SOS + "אני צריכה אדם" | COMPLETE — חי |
