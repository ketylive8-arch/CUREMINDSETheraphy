...
(function () {
  ...
  function OnboardingQuiz({ onComplete, onExit }) {
    const SCRIPT = [
      ...
      { key: "challenge",
        ...
        opts: ["עומס, הצפה רגשית ותחושת עמידה במקום", "חרדת ביצוע, פחד מכישלון או דחיינות", "ביקורת עצמית גבוהה ופרפקציוניזם", "ספק עצמי וחוסר שקט פנימי"] },
      { key: "impact",
        q: "הבנתי אותך. ואיך האתגר הזה משפיע על הניסיון שלך לפעול ולהתקדם ביום-יום?",
        opts: ["משתק אותי לחלוטין", "גורם לי לדחות ולהתרחק", "מייצר עייפות ושחיקה", "גורם לי להטיל ספק בשינוי"] },
      { key: "goal",
    ...
    const INSIGHT =
      "אני כבר רואה את התמונה שלך המערכת שלך פשוט עובדת שעות נוספות כדי להגן עלייך — וזה בדיוק מה שמייצר את התחושה הזו. בשיטת CureMindset נחליף יחד את המאמץ הזה בחיווט מחדש, בעדינות ובקצב שלך.";
    const FINAL =
  ...
  function AuthGate({ onAuthed, onExit }) {
    ...
    // המסלול היחיד שקיים בפועל הוא הליווי הדיגיטלי ב-₪297/חודש (server/db.js) —
    // ההודעה מותאמת אישית לפי האבחון, אבל המחיר תמיד אמיתי ותואם למה שבאמת גובים.
    function recommendFromSummary(summary) {
      const s = String(summary || "");
      if (/ביקורת עצמית|פרפקציוניזם|ספק עצמי|חוסר שקט/.test(s))
        return { name: "₪297", why: "תהליך מלא לחיזוק דימוי עצמי, ביטחון ושקט פנימי" };
      if (/עומס|הצפה|תקיעות|עמידה במקום/.test(s))
        return { name: "₪297", why: "כל המודולים + מעקב וניתוח דפוסים לוויסות ולחוסן" };
      if (/חרדת ביצוע|כישלון|דחיינות/.test(s))
        return { name: "₪297", why: "צ'אט, מודולים ועוגן לרגעי לחץ — בקצב שלך" };
      return { name: "₪297", why: "המסלול המאוזן — תהליך מלא בקצב שלך" };
    }
